...
int TEST_MAIN()
{
    ...
    try {
        ...
        test(__LINE__, {"b","a"}, test_active{1,1,0,0});
        test(__LINE__, {"ab"},    test_active{1,1,0,0});
        test(__LINE__, {"ba"},    test_active{1,1,0,0});
        //second group
        test(__LINE__, {"-a"},      test_active{0,0,1,0});
        test(__LINE__, {"-b"},      test_active{0,0,0,1});
        test(__LINE__, {"-a","-b"}, test_active{0,0,1,1});
        test(__LINE__, {"-b","-a"}, test_active{0,0,1,1});
        test(__LINE__, {"-ab"},     test_active{0,0,1,1});
        test(__LINE__, {"-ba"},     test_active{0,0,1,1});
        test(__LINE__, {"-b-a"},    test_active{0,0,1,1});
        test(__LINE__, {"-a-b"},    test_active{0,0,1,1});
        //fail cases
        ...
        test(__LINE__, {"a-b"},  test_active{});
        test(__LINE__, {"b-a"},  test_active{});
    }
